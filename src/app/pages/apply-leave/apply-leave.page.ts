...
export class ApplyLeavePage implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.employee == '0')
    {
      this.appCommon.presentNoInternetToast('You are still not an employee!');
  ...
  clearFields() {
    ...
    this.reason = '';
    this.leave_type = '';
    this.txt_ref_schema_leave_type = '';
    this.from_date = ''; 
    this.to_date = '';
    this.from_half_day = '';
    this.to_half_day = '';
  ...
  async getLeaveReferenceData() {
    ...
    const modal = await this.modalController.create({
      ...
      componentProps: {
        pop_up_table_name: 'employee_leave_master',
        calling_table_name: 'leave_entry',
        system_name: 'leave_type',
        value: '',
        ref_search_value: 0,
    ...
    modal.onDidDismiss().then((dataReturned) => {   
      ...
      if (dataReturned['data']['internal_code'] > 0) {
        this.leave_type = dataReturned['data']['internal_code'];
        this.txt_ref_schema_leave_type = dataReturned['data']['value'];
        this.GetLeaveBalanceForEmployeeAndLeaveType();
      }
    });
  ...
  GetDateDifference()
{
  let from_half_day = "No"; 
  let to_half_day = "No";
  
  if(this.from_half_day){
    from_half_day = "Yes"; 
  }
  if(this.to_half_day){ 
    to_half_day = "Yes"; 
    ...
    let postData = {
      ...
      'from_half_day':from_half_day,
      'to_half_day':to_half_day,
    };
   
 
    this.http.post(this.appCommon.baseAppUrl + "CalculateNoOfDays", postData)
      .subscribe(response => {
...
GetLeaveBalanceForEmployeeAndLeaveType() 
{
  ...
  let postData = {
    'employee': this.employee,  
    'leave_type':this.leave_type ,
    'effective_date':this.to_date
   
  }; 
  this.http.post(this.appCommon.baseAppUrl + "GetLeavebalanceForEmployeeAndLeaveType", postData)
    .subscribe(response => {

      console.table(response) 

      if (response['error_code'] == '0') { 
        this.leave_balance = (response['data']['cb']);  
      } else
      {
...
SaveApplyLeave() {
    ...
    let postData = {
      ...
      "general": {
          ...
          reason: this.reason,
          leave_type: this.leave_type,
          txt_ref_schema_leave_type: this.txt_ref_schema_leave_type,
          from_date: this.from_date,
          to_date: this.to_date,
          from_half_day: from_half_day,
          to_half_day: to_half_day,
  ...
  nextpage() {
    this.route.navigate(['/view-apply-leave']);
  }
